...
def get_lm_nlp():
    lm_text = lm_nlp.query.order_by(
        lm_nlp.id.desc()).limit(webapp.config.LIMIT_COUNT)
    return lm_text


def get_class_nlp():
    class_nlp_text = class_nlp.query.order_by(
        class_nlp.id.desc()).limit(webapp.config.LIMIT_COUNT)
